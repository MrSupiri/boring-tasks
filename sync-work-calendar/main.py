...
work_cal_token = json.loads(os.getenv("WORK_CALENDAR_TOKEN"))
personal_cal_token = json.loads(os.getenv("PERSONAL_CALENDAR_TOKEN"))
...
def get_work_calendar_event(token):
...
def get_personal_calendar_events(token, calendar_id):
    # Google Calendar API setup
...
def update_calendar(token, personal_cal_id, new_events, changed_events):
    SCOPES = ['https://www.googleapis.com/auth/calendar.events']
    creds = Credentials.from_authorized_user_info(token, SCOPES)
...
if __name__ == '__main__':
    anonymized_events = get_work_calendar_event(work_cal_token)
    
    existing_events = get_personal_calendar_events(personal_cal_token, personal_cal_id)
    
    new_events, changed_events = calculate_changes(anonymized_events, existing_events)

    update_calendar(personal_cal_token, personal_cal_id, new_events, changed_events)